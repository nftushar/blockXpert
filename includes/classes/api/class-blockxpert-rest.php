...
class BlockXpert_REST
{
    ...
    public function register_rest_routes()
    {
        register_rest_route('blockxpert/v1', '/generate-faq', [
            ...
            'permission_callback' => [$this, 'has_rest_permission'],
        ]);
    ...
    {
        $authorization_error = $this->ensure_request_authorized();
        ...
        $response = $this->request_openai_completion($openai_url, $apiKey, $body);
    ...
    private function has_rest_permission()
    {
        return current_user_can('edit_posts') && wp_verify_nonce($this->get_request_nonce(), 'wp_rest');
    }

    private function ensure_request_authorized()
    {
        if (!wp_verify_nonce($this->get_request_nonce(), 'wp_rest')) {
    ...
    {
        return sanitize_text_field(wp_unslash($_REQUEST['_wpnonce'] ?? ''));
    }

    private function request_openai_completion($url, $api_key, array $body)
    {
